...
for nu in eachindex(nu_val), tau in eachindex(tau_val)
    local params = [tau_val[tau],0.01,2,nu_val[nu]]; #(τ, v0, n, η) might work: params = [0.1,0.02,3,0.1]
    local bg = BehaviourGrid(params,dataset); 
    # p=plot_behaviour_grid(bg,start_s_slice=100,idx_axis=false,xguide="D",yguide="S")
    local p=plot_behaviour_grid(bg,start_s_slice=100,idx_axis=false)
    # tau=parse(Int, "$tau_val") 
    # nu=parse(Int, "$nu_val") 
    # Only show y-axis labels for the leftmost plots (column 1)
    if tau==1 && nu==1
       plot!(p,ylabel="S & nu:"*string(nu_val[nu]),xtick=false,title="tau:"*string(tau_val[tau]))
    elseif tau==1 && nu==length(nu_val)
        plot!(p,xlabel="D",ylabel="S & nu:"*string(nu_val[nu]),title="v0=0.01 n=2") 
    elseif tau==1 && nu!=length(nu_val[nu])
       plot!(p,ylabel="S & nu:"*string(nu_val[nu]),xtick=false)
    elseif tau!=1 && nu==length(nu_val)
